...
void Optimizer::AdamCompute(float* __restrict p, float* __restrict m, float* __restrict v, const float* __restrict d, size_t size, float lr, size_t n, float b1, float b2, float epsl, size_t t) {
    ...
    const __m512 _b2 = _mm512_set1_ps(b2);
    const __m512 _nb1 = _mm512_set1_ps(1.0f-b1);
    const __m512 _nb2 = _mm512_set1_ps(1.0f-b2);
    const __m512 _b1t = _mm512_set1_ps(1.0f-std::pow(b1, (float)t));
    const __m512 _b2t = _mm512_set1_ps(1.0f-std::pow(b2, (float)t));
    ...
    for (ssize_t i = 0; i <= ((ssize_t)size)-16; i += 16) {
        ...
        const __m512 _m2 = _mm512_mul_ps(_m, _b1);
        const __m512 _m3 = _mm512_fmadd_ps(_nb1, _d, _m2);

        const __m512 _d2 = _mm512_mul_ps(_d, _d);
        const __m512 _v2 = _mm512_mul_ps(_v, _b2);
        const __m512 _v3 = _mm512_fmadd_ps(_nb2, _d2, _v2);

        const __m512 _mh = _mm512_div_ps(_m3, _b1t);
        const __m512 _vh = _mm512_div_ps(_v3, _b2t);

        const __m512 _v4 = _mm512_add_ps(_vh, _epsl);
        const __m512 _v5 = _mm512_sqrt_ps(_v4);

        const __m512 _m4 = _mm512_div_ps(_mh, _v5);
        const __m512 _res = _mm512_fnmadd_ps(_factor, _m4, _p);

        _mm512_store_ps(&p[i], _res);
        _mm512_store_ps(&m[i], _m3);
        _mm512_store_ps(&v[i], _v3);
    }
